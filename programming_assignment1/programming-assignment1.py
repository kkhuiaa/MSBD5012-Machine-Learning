...
"""
     CSIT6000G/MSBD5012 Programming Assignment 1
         Due Date: Oct 11 (CSIT), Oct 12 (MSBD)
To be submitted via canvas. Instructions on Annoucement Page.
...
def compute_softmax_loss(W, X, y, reg):
    ...
    # regularization!                                                           #
    #############################################################################
    train_count = X.shape[0] #number of training data

    #Calculate the score
    s = X.dot(W)
    shifted_s = s - np.max(s, axis=1)[:, np.newaxis] #Substract the max for numerical stability (to make all the values negative)
    softmax_prob = np.exp(shifted_s)/np.sum(np.exp(shifted_s), axis=1)[:, np.newaxis] #Calculate the softmax prob

    #Calculate the loss for this sample data
    softmax_loss = -np.sum(np.log(softmax_prob[range(0, train_count), y]))
    loss = softmax_loss/train_count + reg*np.sum(W**2)
    
    #Calculate the dW
    softmax_prob[range(train_count), y] -=1 # -1 when it is labeled as correct class
    dW = X.T.dot(softmax_prob)/train_count + 2*W*reg #the gradient
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
